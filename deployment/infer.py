...
t0=time.time()
trt_engine = TRT_engine("./trt_model/best.engine")
path = "./pictures/milk/image"
files = os.listdir(path)
for file in files:
    img = cv2.imread(path + "/" + file)
    results = trt_engine.predict(img, threshold=0.5)
    img = visualize(img, results)
    cv2.imwrite("./pictures/milk/result/" + file, img)

print(f" The image with the result is saved in: {path}")
